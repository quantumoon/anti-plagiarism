...
class LevenshteinDist():
    ...
    def _get_similarity(self, codes_pair, weight_for_docs=0.1):
        """
        ...
        return (1 - self.dist(code1, code2) / max(len(code1), len(code2)) + 1 - weight_for_docs * self.dist(docs1, docs2) / max(len(docs1), len(docs2))) / 2
...
def get_prediction(codes_pair, clf):
    ...
    return (preds[0][0] * preds[1][1] + preds[0][1] * preds[1][0]) / 2
